...
All Azure-generated names below are computed at deploy time from
`uniqueString(resourceGroup().id)` in `infra/main.bicep`. Treat them as
implementation detail and look them up in the Azure portal for your
environment rather than relying on the literal values.

| Resource | Bicep name | SKU / tier | Purpose |
|---|---|---|---|
| Container App | `<base>-api` | Consumption (scales 0–2) | Hono API. Public ingress on port 3001. System-assigned MSI. |
| Container Apps Environment | `<base>-cae` | Linked to Log Analytics | Hosts the Container App. |
| Static Web App (kiosk) | `<base>-kiosk` | Free | Hosts the kiosk SPA. Custom domain `ziggy.expertslive.dev`. SWA fallback hostname is auto-generated. |
| Static Web App (admin) | `<base>-admin` | Free | Hosts the admin SPA. Custom domain `ziggy-admin.expertslive.dev`. SWA fallback hostname is auto-generated. |
| Cosmos DB | `<base>-db-<suffix>` | Serverless, North Europe | Database `ziggy`, ~9 containers (events, sponsors, sponsor-tiers, floor-maps, admins, i18n-overrides, booth-overrides, shop-items, analytics, audit-log, auction-bids). All non-event/non-admin containers partition on `/eventSlug`. |
| Storage Account | `<base>st<suffix>` | Standard LRS | Blob container `images` with public-blob access for sponsor logos and floor map images. Private `snapshots` container for backup JSONs. |
| Container Registry | `<base>cr<suffix>` | Basic, ACR admin user enabled | Holds `ziggy-api` images. |
| Key Vault | `<base>-kv-<suffix>` | Standard | Holds `jwt-secret` and `run-events-api-key`. Container App MSI granted "Key Vault Secrets User" via role assignment. |
| Log Analytics workspace | `<base>-logs` | PerGB2018, 30-day retention | Container App stdout. |

`<base>` defaults to `ziggy`; `<suffix>` is a deterministic 13-char hash
...
- **Identity:** `type: 'SystemAssigned'`. The principal is granted "Key Vault
  Secrets User" on the Key Vault (`kvSecretsUserRole` resource).
...
| `ACR_NAME` | ACR resource name for `az acr build` (auto-generated by Bicep — read it from the Azure portal or `az acr list`). |
...
az containerapp update \
  --name ziggy-api \
  --resource-group "$RG" \
  --image "$ACR.azurecr.io/ziggy-api:$(git rev-parse --short HEAD)"
```

(Where `RG` is your resource group, e.g. `export RG=ziggy-rg` matches our
prod env.)
...
its name as a parameter (`keyVaultName`, supplied by the deploy command).
