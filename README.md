...
- Kiosk: `https://ziggy.expertslive.dev`
- Admin: `https://ziggy-admin.expertslive.dev`

(The Static Web Apps fallback hostnames are auto-generated by Azure and
referenced from `infra/main.bicep` + the SWA config files at deploy time.)

## Packages at a glance

- `@ziggy/shared` — TypeScript types + constants used across the rest
- `@ziggy/api` — Hono backend (Container Apps), proxies run.events with
  in-memory caching, serves admin-managed Cosmos data, ingests anonymous
  analytics, hosts the auction endpoints
- `@ziggy/kiosk` — public touch-screen SPA (Static Web App)
- `@ziggy/admin` — admin SPA (Static Web App): dashboard, sponsors,
  shop items, floor maps + hotspot editor, image library, snapshots,
  trash, audit log, analytics, kiosk preview, admin user mgmt, auction
